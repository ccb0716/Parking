...
def bing_tu( x, y,colors):#,c=0,d=0,e=0,f=0,g=0,h=0
    ...
    # 生成图像...
    plt.savefig('./Visual/bing_tu.Visual',dpi=300)  # 保存图像到文件
    plt.close('all')
def bing_tu_1( ):#,c=0,d=0,e=0,f=0,g=0,h=0
    result = service.query(
        "select wel from car where  state=?", 1)
    ...
    # 生成图像...
    plt.savefig('./Visual/bing_tu_2.Visual',dpi=300)  # 保存图像到文件
    plt.close('all')
def zhu_zhuangtu():
    result = service.query(
        "select time from car where  state=? ", 1)
    ls = [0, 0, 0, 0, 0, 0]
    # 类统计
    for i in result:
    ...
    plt.savefig('./Visual/zhuzhuang_tu.Visual',dpi=300)
    plt.close('all')
bing_tu_1()
...
def fee_plt():#使用ax,而不是plt
    ...
    y1=[]
    result = service.query(
        "select in_time,fee from car where  state=? ", 1)
    data = list(result)
    # 创建一个字典来分组日期和费用
    date_dict = {}
    # 遍历数据并计算相同日期的费用总和
    ...
    plt.savefig('./Visual/fee.Visual',dpi=300)
    plt.close('all')
def shiduan():
    result = service.query(
        "select Time_period from car where  state=? ", 1)
    x,y=[],[]
    ...
    plt.savefig('./Visual/shiduan.Visual', dpi=300)
    plt.close('all')
def jiedai():
    x = []
    y1 = []
    ...
    plt.savefig('./Visual/cc.Visual', dpi=300)
    plt.close('all')
